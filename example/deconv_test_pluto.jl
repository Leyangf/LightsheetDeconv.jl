...
function plot_intensity_profile(intensity_map; axis=:y, title_text="Intensity Profile")
    ...
    if axis == :y
        axis_range = range(-129*0.13, 128*0.13, length=sz_x)  # 设置正确的物理范围
        intensity_profile = intensity_map[:, div(sz_y, 2)]
    elseif axis == :x
        axis_range = range(-129*0.13, 128*0.13, length=sz_y)  # 设置正确的物理范围
...
Plots.heatmap(h_ill[45:84, :], color=:viridis, xlabel="Dimension 1 (sz[1])", ylabel="Dimension 3 (sz[3])", title="Visualization of h_ill", aspect_ratio=:equal)
...
begin
	...
	p0 = Plots.heatmap(h_ill_g[45:84, :]; heatmap_opts...)
    p1 = Plots.heatmap(h_ill[45:84, :]; heatmap_opts...)
    p2 = Plots.heatmap(h_ill_re1[45:84, :]; heatmap_opts...)
    p3 = Plots.heatmap(h_ill_re2[45:84, :]; heatmap_opts...)
...
# ╔═╡ b81b86c3-4f21-4331-97ef-f5fa5479755c
begin
...
begin
	corner_bead_position = (15, sz[2] ÷ 2, sz[3] ÷ 2)  # Near corner, but slightly offset to avoid out-of-bounds
...
begin
    ...
    center_bead_small_blur = Float32.(crop_subregion(center_bead_full_blur, sub_sz, center_bead_position))
	center_bead_small_blur = center_bead_small_blur ./ maximum(center_bead_small_blur)
	volume(center_bead_small_blur)
end

# ╔═╡ 1a1c69b0-fe4a-432e-b9e3-55300367704b
function visualize_3d_slices(sample::Array{Float32, 3})
    # 计算中心切片索引
    center_x = size(sample, 1) ÷ 2
    center_y = size(sample, 2) ÷ 2
    center_z = size(sample, 3) ÷ 2

    # 计算坐标轴刻度 (单位: μm)
    x_ticks = (0:size(sample, 1)-1) .* 0.13 .- center_x * 0.13
    y_ticks = (0:size(sample, 2)-1) .* 0.13 .- center_y * 0.13
    z_ticks = (0:size(sample, 3)-1) .* 0.13 .- center_z * 0.13

    # 只显示 -1, 0, 1 三个刻度
    tick_values = [-1.5, -0.75, 0.0, 0.75, 1.5]

    # 选取三个正交切片
    slice_xy = sample[:, :, center_z]  # z方向中心切片
    slice_xz = sample[:, center_y, :]  # y方向中心切片
    slice_yz = sample[center_x, :, :]  # x方向中心切片

    # 可视化切片
    p1 = Plots.heatmap(x_ticks, y_ticks, slice_xy, color=:viridis,
                        xlabel="X (μm)", ylabel="Y (μm)", xticks=(tick_values, string.(tick_values)), 
                        yticks=(tick_values, string.(tick_values)))
    
    p2 = Plots.heatmap(x_ticks, z_ticks, slice_xz', color=:viridis,
                        xlabel="X (μm)", ylabel="Z (μm)", xticks=(tick_values, string.(tick_values)), 
                        yticks=(tick_values, string.(tick_values)))
    
    p3 = Plots.heatmap(y_ticks, z_ticks, slice_yz', color=:viridis,
                        xlabel="Y (μm)", ylabel="Z (μm)", xticks=(tick_values, string.(tick_values)), 
                        yticks=(tick_values, string.(tick_values)))

   Plots.plot(p1, p2, p3, layout=(1, 3), size=(1200, 400), aspect_ratio=:equal, 
               left_margin=5Plots.mm, bottom_margin=0Plots.mm, top_margin=0Plots.mm,
               subplot_spacing=0.01)

end

# ╔═╡ 48bf90ca-8dfc-479d-bf23-01888af7e136
visualize_3d_slices(center_bead_small_blur)
...
begin
    ...
    corner_bead_small_blur = Float32.(crop_subregion(corner_bead_full_blur, sub_sz, corner_bead_position))
	corner_bead_small_blur = corner_bead_small_blur ./ maximum(corner_bead_small_blur)
    volume(corner_bead_small_blur)
end

# ╔═╡ b8da5ece-63ae-40da-9ca3-26d1dc4e75a7
visualize_3d_slices(corner_bead_small_blur)
...
PlutoTest = "~0.2.2"
PlutoUI = "~0.7.60"
PointSpreadFunctions = "~0.4.2"
SyntheticObjects = "~1.0.0"
"""
...
manifest_format = "2.0"
project_hash = "96a9d153f7761f328c8b3211b552b0bc23c507fe"
...
[[deps.PointSpreadFunctions]]
deps = ["FFTW", "FourierTools", "IndexFunArrays", "NDTools", "QuadGK", "SpecialFunctions", "ZernikePolynomials"]
git-tree-sha1 = "c813d40da54d0c4fb0b0a100e5025b1e2805de04"
uuid = "e8810a93-244e-46c5-8da3-35c5dd956001"
version = "0.4.3"
...
[[deps.SyntheticObjects]]
deps = ["Cairo", "IndexFunArrays", "Random"]
git-tree-sha1 = "056d787e614907b230f380d0c9600642ecf8bd9d"
uuid = "e7028c27-0967-45e9-8fdb-dbc10ccb2b0a"
version = "1.0.0"
...
[[deps.ZernikePolynomials]]
git-tree-sha1 = "2dd6619de48f7b79b51d1651a63f2a4c4b27e6b5"
